...
const fileteredCities = (walker) => {
    const citiesArrayAssignements = []
    for (const walkerCity of walkerCities){
        if (walkerCity.walkerId === walker.id){
            citiesArrayAssignements.push(walkerCity)
        }
    }
    return citiesArrayAssignements
}

const assignedCities = (assignmentsArray) => {
    let matchingCities = ""
            ...
            for (const walker of walkers) {
                ...
                */
                if (walker.id === parseInt(walkerId)) {
                    const assignments = fileteredCities(walker)
                    const cities = assignedCities(assignments)
